...
def parse_toml(d: dict) -> tuple[EvalRequest, str, dict]:
    green = d.get("green_agent")
    if not isinstance(green, dict) or "endpoint" not in green:
        raise ValueError("green_agent.endpoint is required in TOML")
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: python -m agentbeats.client_cli <scenario.toml> [output.json]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\nError communicating with green agent: {e}")
        sys.exit(1)
    ...
    if output_path:
        ...
        output_data = {"participants": role_to_id, "results": all_data_parts}
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(json.dumps(output_data, indent=2))
        print(f"Results written to {output_path}")

    # Always print the final artifact summary to stdout
    if artifacts:
        print("\n--- Evaluation Results ---")
        for artifact in artifacts:
            ...
            for d in data_parts:
                ...
                total = d.get("total")
                if acc is not None:
                    print(f"Accuracy: {acc:.1%} ({passed}/{total})")
                    for r in d.get("results", []):
                        status = "PASS" if r.get("passed") else "FAIL"
                        err = f"  error: {r['error']}" if r.get("error") else ""
                        print(f"  [{status}] {r.get('instance_id', '?')}{err}")
